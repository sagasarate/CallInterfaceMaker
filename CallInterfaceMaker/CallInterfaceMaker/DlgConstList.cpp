...
BEGIN_MESSAGE_MAP(CDlgConstList, CDialog)
	...
	ON_BN_CLICKED(IDOK, &CDlgConstList::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BUTTON_SORT, &CDlgConstList::OnBnClickedButtonSort)
END_MESSAGE_MAP()
...
void CDlgConstList::OnBnClickedButtonMove()
{
	...
	while(Pos)
	{
		Item = m_lvList.GetNextSelectedItem(Pos);
		UINT Index = (UINT)m_lvList.GetItemData(Item);
		if(Index<m_ConstDefineList.size())
		{
	...
	if(MoveList.size())
	{
		...
		FillList();
		if (Item >= m_lvList.GetItemCount())
			Item = m_lvList.GetItemCount() - 1;
		m_lvList.EnsureVisible(Item, true);
	}
}


void CDlgConstList::OnBnClickedOk()
{
	// TODO:  在此添加控件通知处理程序代码
	CDialog::OnOK();
}
...
void CDlgConstList::OnBnClickedButtonSort()
{
	// TODO:  在此添加控件通知处理程序代码
	qsort(m_ConstDefineList.begin()._Ptr, m_ConstDefineList.size(), sizeof(CONST_DEFINE_INFO), CCallInterfaceMakerDlg::ConstComp);
	FillList();
}
