	...
	for(size_t j=0;j<m_DataObjectModifyFlags.size();j++)
	{
		...
		for(size_t i=0;i<DataObjectList.size();i++)
		{
			...
			if(pModifyFlagUnit)
			{
				...
				CheckModifyFlagUnitMembers(DataObjectList, ModifyFlagUnits[ModifyFlagUnits.size()-1]);
			}
			else
			{
...
void CDlgDataObjectModifyFlagManager::CheckModifyFlagUnitMembers(vector<STRUCT_DEFINE_INFO *>& DataObjectList, MODIFY_FLAG_UNIT& DataObjectModifyFlag)
{
	for (size_t i = 0; i < DataObjectList.size(); i++)
	{
		if (DataObjectList[i]->Name == DataObjectModifyFlag.ClassName)
		{
			for (int j = (int)DataObjectModifyFlag.Members.size() - 1; j >= 0; j--)
			{
				bool IsExist = false;
				for (size_t k = 0; k < DataObjectList[i]->MemberList.size(); k++)
				{
					if (DataObjectList[i]->MemberList[k].Name == DataObjectModifyFlag.Members[j])
					{
						IsExist = true;
						break;
					}
				...
				if (!IsExist)
				{
					DataObjectModifyFlag.Members.erase(DataObjectModifyFlag.Members.begin() + j);
				}
			}
		}
	}
