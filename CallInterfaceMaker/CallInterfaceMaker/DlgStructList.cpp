...
BEGIN_MESSAGE_MAP(CDlgStructList, CDialog)
	...
	ON_BN_CLICKED(IDOK, &CDlgStructList::OnBnClickedOk)
	ON_BN_CLICKED(IDC_BUTTON_SORT, &CDlgStructList::OnBnClickedButtonSort)
END_MESSAGE_MAP()
...
void CDlgStructList::OnBnClickedButtonMove()
{
	...
	while(Pos)
	{
		Item=m_lvStruct.GetNextSelectedItem(Pos);
		UINT Index=(UINT)m_lvStruct.GetItemData(Item);
		if(Index<m_StructDefineList.size())
		{
	...
	if(MoveList.size())
	{
		...
		FillList();
		if (Item >= m_lvStruct.GetItemCount())
			Item = m_lvStruct.GetItemCount() - 1;
		m_lvStruct.EnsureVisible(Item, true);
	}	
}


void CDlgStructList::OnBnClickedOk()
{
	// TODO:  在此添加控件通知处理程序代码
	CDialog::OnOK();
}
...
void CDlgStructList::OnBnClickedButtonSort()
{
	// TODO:  在此添加控件通知处理程序代码
	qsort(m_StructDefineList.begin()._Ptr, m_StructDefineList.size(), sizeof(STRUCT_DEFINE_INFO), CCallInterfaceMakerDlg::StructComp);
	FillList();
}
